...
<script>
    jQuery(document).ready(function(){
        ...
        let currIndex = 0;
        $("#announceTab li").eq(currIndex).find('a').addClass("active");
        $(".changableContent").eq(currIndex).show();
        $("#announceTab li").click(function(){
            $(this).get(0).scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
            $(".teamTabs li a").removeClass("active");
            $(this).find('a').addClass("active");
            
            currIndex = $("#announceTab li").index(this);

            $(".changableContent").hide();
            $(".changableContent").eq(currIndex).show();
            
            $(".loading_wrapper").show();
            setTimeout(() => {
                $(".loading_wrapper").hide();
            }, 2000); // remove when ajax will setup
            // $.ajax({
            //     method: 'POST',
            //     url: '/wp-admin/admin-ajax.php',
            //     data: {
