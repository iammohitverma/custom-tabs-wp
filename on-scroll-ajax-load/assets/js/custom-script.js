...
// init Isotope
  ...
  $(".filter-button-group").on("click", "button", function () {
    ...
    sessionStorage.setItem("activeTab", currActiveWithoutDot);
  ...
  jQuery(window).scroll(function(){
    ...
    let progressWrapOffsetTop = jQuery(".progress_wrapper").offset().top;
    if((isScroll == true) && ((windowHeight + windowScrollTop) >= progressWrapOffsetTop)){
      loadMoreFun();
      isScroll = false;
    }
  });
  ...
  // init Isotope with selected value 
  let selectedTab = sessionStorage.getItem("activeTab");
  if(selectedTab){
    jQuery(".filter-button-group button").removeClass("is-checked");
    jQuery(`[data-filter='.${selectedTab}']`).addClass("is-checked");
    $grid.isotope({ filter: `.${selectedTab}`});
    ajaxRunOnClickFun(selectedTab);
    currActiveWithoutDot = selectedTab;
  }
