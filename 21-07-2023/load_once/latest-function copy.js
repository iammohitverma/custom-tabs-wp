jQuery(document).ready(function () {
    ...
    if (isClassOnce) {
        let currUrl, lastIndexOfBS, currUrlPageSlug;
        currUrl = window.location.href;
        function getPageSlug() {
            lastIndexOfBS = currUrl.lastIndexOf('/');
            currUrlPageSlug = currUrl.substring(lastIndexOfBS + 1);
        }
        getPageSlug();
        if (currUrlPageSlug == "") {
            currUrl = currUrl.substring(1, currUrl.length - 1);
            getPageSlug(); //check once again 
        }
        ...
        if (getloadOnceObj == null) {
            arrOfMulitplePages.push({ currUrlPageSlug: currUrlPageSlug });
            ...
            for (const key in getloadOnceObj) { //check in obj 
                if ((getloadOnceObj[key].currUrlPageSlug) == currUrlPageSlug) {
                    availInLoadObj = true;
                    break;
                }
            }
            if (availInLoadObj == true) {
                ...
                arrOfMulitplePages.push(...getloadOnceObj, { currUrlPageSlug: currUrlPageSlug });
                getloadOnceStr = JSON.stringify(arrOfMulitplePages)
                localStorage.setItem("loadOnce", getloadOnceStr);
